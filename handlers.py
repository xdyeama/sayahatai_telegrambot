...
async def handle_cities_choice_state(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    global_cities_choice.add(cb.data)
...
async def handle_generate_trip_cb(cb: CallbackQuery, state: FSMContext):
    check = await db.check_balance(cb.message.from_user.id)
    if not check:
        return await cb.message.answer(texts.balance_error, reply_markup=kb.iexit_kb)
    context = await db.get_request_response(cb.message.from_user.id)
    message = texts.main_prompt_template.format(
        cities=global_cities_choice,
    ...
    await db.update_request_response(cb.message.from_user.id, cb.message.text, res[0])
    await cb.message.answer(
        res[0], disable_web_page_preview=True, reply_markup=kb.exit_kb
    )
    await db.update_balance(cb.message.from_user.id, res[1])


@router.message(GenStateGroup.generate_trip_state)
@flags.chat_action("typing")
